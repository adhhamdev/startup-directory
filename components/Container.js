...
const Container = ({ startups, searchQuery, industries, investmentTypes }) => {
    ...
    const renderStartups = () => {
        ...
        if (selectedIndustry !== "All") {
            filteredStartups = filteredStartups.filter(startup => startup.IndustryVertical === selectedIndustry);
        }
        if (selectedInvestmentType !== "All") {
            filteredStartups = filteredStartups.filter(startup => startup.InvestmentType === selectedInvestmentType);
        }
        return filteredStartups.map(startup => <Card key={startup._id} startup={startup} handleModal={handleModal} />);
    }
    ...
    return (
        <div>
            ...
            {isModalOpen && <Modal startup={modalStartup} setIsModalOpen={setIsModalOpen}/>}
        </div>
    );
};
