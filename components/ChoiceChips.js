const ChoiceChips = ({
  ...
  const distinctInvestmentTypes = investmentTypesList.filter(
    (value, index, self) => {
      const lowerCaseValue = value.toLowerCase();
      return (
        self.findIndex((item) => item.toLowerCase() === lowerCaseValue) ===
        index
      );
    },
  );
      ...
      {distinctInvestmentTypes.map((investType, index) => (
        <button
          ...
          className={`px-5 py-2 text-sm rounded-full ${
            selectedInvestmentType === investType
              ? 'bg-blue-500 text-white'
              : 'bg-gray-200 text-gray-700'
          } hover:bg-blue-500 hover:text-white mx-1`}
